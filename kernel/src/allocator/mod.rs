...
fn memory_map() -> Option<(usize, usize)> {
    let binary_end = unsafe { (&_end as *const u8) as u32 } as usize;

    let atag = Atags::get().find(|&atag| match atag {
        Atag::Mem(_) => true,
        _ => false,
    })?;

    let mem = atag.mem()?;
    let physical_end = (mem.start + mem.size) as usize;
    Some((binary_end, physical_end))
}
